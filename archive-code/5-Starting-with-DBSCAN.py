...
model = hdbscan.HDBSCAN(min_cluster_size=mcs, min_samples=min_pts)
model.fit(datanp)
tps2 = time.time()
...
plt.title("Données après clustering DBSCAN (1) - Min cluster sizes= "+str(mcs)+" MinPts= "+str(min_pts)+ "\n"+ "nb clusters= "+str(n_clusters) + "; nb noise= "+str(n_noise))
plt.show()
